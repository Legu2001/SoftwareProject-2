...
dist_ema = (1.0 - _EMA_ALPHA) * dist_ema + _EMA_ALPHA * dist_raw;

  // adjust servo position according to the USS read value
  // add your code here!

  if (dist_ema >= 180.0 && dist_ema <= 360.0) {
    int servo_position = map(dist_ema, 180, 360, _DUTY_MIN, _DUTY_MAX);
...
// myservo.writeMicroseconds(((dist_raw - _TARGET_LOW) / (_TARGET_HIGH - _TARGET_LOW)) * (_DUTY_MAX - _DUTY_MIN) + _DUTY_MIN);
